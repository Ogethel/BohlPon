...
{
    /// <summary>
    /// The RNGNeedsCore class provides static functionalities and properties that cater to random number generation and selection methods.
    /// </summary>
                ...
                if (list.MaintainPickCountIfDisabled)
                {
                    ...
                    if (m_EnabledItems.Count == 1)
                    {
                        ...
                        for (var index = 0; index < itemCount; index++)
                        {
                            ...
                            if (!probabilityItem.Enabled) continue;
                            for (var i = 0; i < pickCount; i++) result.Add(index);
                            return;
                        }
                    ...
                    {
                        RLogger.Log(
                            "Total enabled probability of the list is too low to Maintain selected Pick Count. This could result in a very long selection process or a hang. No items were picked. Enable more items, lower the pick count, or turn off 'Maintain Pick Count'. This can also happen in influenced list - make sure -1 influence spread of at least one item is above 0%",
                            LogMessageType.Hint);
                        result = new NativeList<int>(Allocator.TempJob);
                        return;
                    }
